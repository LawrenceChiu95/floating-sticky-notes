...
- 将单张便签收起为仍留在桌面、保持置顶并可拖动的标题横条；横条还可以吸到屏幕左右边缘，收成一枚半藏的书签头，再拖出来展开
- 粘贴剪贴板图片、系统截图，或直接拖入图片
- 点击图片打开独立预览窗口，窗口可自由移动和调整大小，图片可缩放、平移并在多图间切换
- 分别调整每张便签的颜色和透明度
- 恢复窗口大小与位置
...
当前正式版本为 [`0.1.21`](https://github.com/LawrenceChiu95/floating-sticky-notes/releases/tag/v0.1.21)。本版本修复 Windows 启动时恢复贴边便签可能出现的异常，并改善拖出后未能展开时的处理，避免书签头自行反复缩回屏幕边缘。

Windows x64 Setup 和 Apple Silicon Mac DMG 可从 [`floating-sticky-notes-updates v0.1.21`](https://github.com/LawrenceChiu95/floating-sticky-notes-updates/releases/tag/v0.1.21) 下载。Windows 可以在应用内完成检查、下载和重启安装；macOS 会下载并校验 DMG、打开安装镜像，用户仍需手动拖到 Applications 并确认替换。Windows 安装包未签名；macOS 构建使用完整的 ad-hoc 签名，但未使用 Apple Developer ID 且未经过公证，首次从外部下载时需要在“系统设置 → 隐私与安全性”中选择“仍要打开”。
...
开发模式使用独立的 `floating-sticky-notes-dev` 数据目录（与正式版数据目录同级），不会影响同机正式版数据，两边可以同时运行。

项目仍以 npm 10 安装依赖；安全审计使用 npm 11，是因为 npm 10 的旧审计端点正在退役。修改依赖或 `overrides` 时，完整的 lockfile 再生成与兼容验证流程见 [贡献指南](CONTRIBUTING.md#依赖维护)。
...
在 macOS 上构建完整 ad-hoc 签名、但未经 Apple 公证的 Apple Silicon DMG：
