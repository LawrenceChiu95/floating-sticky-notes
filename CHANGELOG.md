...
- 修复 Windows 启动时恢复贴边便签可能出现的异常。
- 改善贴边便签拖出后的失败处理：如果未能展开，书签头会留在拖出的位置，方便再次拖动，不再自行反复缩回屏幕边缘。
...
- 长便签贴到屏幕边上再拖出来，正文会停在贴边前的位置，不再每次回到顶部。
- 手动检查更新失败时，会区分代理/VPN 连不上和连接超时，不再一律提示“检查网络”。
- 检查更新时，如果当前代理或 VPN 连不上，会改走直连再试一次；连接超时或被中断也会自动再试一次。

## [0.1.19] - 2026-09-02

### 新增

- 收起的标题横条可以吸附到当前屏幕的左或右边缘，收成一枚便签颜色的书签头：静止时半藏在屏幕外，鼠标悬上去整条滑出；随时可以把书签头拖回桌面，在落点展开成完整便签。贴边状态会记住，重启后贴回原来那块屏的同一条边；多张便签贴同一侧会自动沿边错开。

### 修复

- 从托盘打开当前版本说明时，窗口会按最近两个版本收紧；更早的版本仍可往下滚动查看。
...
- Windows 自动更新下载期间显示独立、非阻塞的进度窗口。
- 没有百分比时，进度窗口会显示明确的进行中状态。
...
- Windows 现在限制为单实例运行。
- 关闭便签后可从托盘“显示所有便签”，再次启动应用也会恢复已关闭的便签窗口。
