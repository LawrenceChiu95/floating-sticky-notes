...
收起横条还有第三态「贴边」：窗口状态机为 `expanded → collapsed（40px 横条）→ docked（左右工作区边缘的 96×32 横条彩色书签头，有名字横排露出一小段）`，由同一个收起控制器编排，几何变化只做非动画 `setBounds`（禁止 `setBounds(..., true)` 与 `.note-shell` 基础态尺寸过渡）。贴边只允许从横条进入，展开只允许从贴边进入，不支持展开直贴或贴边回横条。三个态的窗口拖动都走原生 `-webkit-app-region: drag`，与系统拖窗同一条路径；自定义拖窗（renderer 指针 + IPC `setPosition`、或主进程 16ms 轮询 `getCursorScreenPoint`）在 Mac 真机上都被判定为黏/卡顿，已整体拆除，不允许以任何形式回来（也没有 start/move/finish 拖动 IPC、没有 offer/accept 会话）。贴边与展开是磁吸：主进程在窗口 `move` 事件上看当前矩形，横条左/右缘探出工作区对应缘 8px 立即 `setDocked({kind:'dock'})` 收成书签头，书签头水平离边 ≥48px 且光标在窗内（挡 macOS show 时回拉屏右缘窗口的非拖动 move）立即 `setDocked({kind:'expand'})` 长开（左贴边往右长、右贴边往左长，夹进工作区）。未过阈值的钉回时机实验中：原生拖拽会话进行中的 move 流写窗写中（位置被重置、位移攒不到 48px 阈值）与被吞（抽搐/定格变形）都坏，2026-08-19 工作区代码改为 move 流不写窗、停歇 180ms 后一次性钉回校验——手感未过验收（bug 2 未解），可能返工。没有「松手判定」（macOS 的 `moved` 是 `move` 的别名，平台没有拖动结束事件），也没有 240ms 过渡（「松手后先演动画再改窗口」同样在真机被否）。磁吸过渡的异步持久化未完成时忽略后续 move，避免重入。几何落定后主进程发 `sticky-notes:dock-applied`（`{dock: {side} | null}`），renderer 只切 `dock` state：贴边渲染书签头、展开渲染完整便签。沿边滑动时 `dock.y` 经 bounds 变化防抖持久化跟随当前位置。几何阈值与矩形计算全部在 `shared/note-dock.ts`，主进程与测试共用。持久化只写 `NoteRecord.dock?: { side, y, x? }`（x 为静止位 x：多屏同名边 y 常打平，恢复靠 x 认屏；旧记录无 x 退化为 y 距离猜屏）；`bounds` 始终表示最后一次展开态的位置与尺寸，进入贴边、沿边滑动都不改写 `bounds`，从贴边展开后用夹取过的展开矩形写回并删除 `dock`。启动时合法 `dock` 先按普通窗口创建、趁 show:false 改成贴边几何（出生即小条 + 不可缩放 + 整页拖窗区的窗口在 macOS 收不到任何 OS 鼠标事件），恢复候选按 |候选.x − dock.x| 认屏，夹取后不落在任何可见工作区则丢弃 `dock` 按 `bounds` 展开；建贴边窗时主进程把 side 写进页面 URL query，preload 同步暴露，renderer 首个 state 初始值就是贴边态——首帧 DOM 就是书签头，不会先挂完整便签再切换。多张同侧书签头即将重叠时沿边向下错开 40px（签高 32 + 8 间隙），不够再向上，仍无空位保证至少 24px 可抓高度。悬停探头：静止半藏（露 48px）、悬上整条 96px 滑出、移开缩回——no-drag 外壳做悬停传感环（drag 区在应用非激活时收不到 OS enter/leave），内嵌 5px 透明 drag 芯做拖动面；enter/leave 只是扳机，主进程用一次性光标查询判定真实悬停，露出期每 120ms 光标复核兜底 OS leave 丢失；探头滑行途中抢拖第一帧偏廊即熔断。贴边不做截图排除 API、点击展开、快捷键或吸顶/底边；设计真源是 [`docs/design/edge-dock.md`](design/edge-dock.md)。2026-08-19 Mac 真机过验收：96×32 横书签头外形、悬停探头、拖出展开、多屏恢复；bug 2（快速拖贴边抽搐/变形）两轮修复未过验、未解，等重新诊断。代码只在 `feat/issue-24-edge-dock`，未合 main、未发布。
