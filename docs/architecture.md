...
收起横条还有第三态「贴边」：窗口状态机为 `expanded → collapsed（40px 横条）→ docked（左右工作区边缘的 96×32 横条彩色书签头）`，由同一个收起控制器编排。贴边只允许从横条进入，展开只允许从贴边进入。三个态的窗口拖动都走原生 `-webkit-app-region: drag`；自定义拖窗已拆除。吸附与展开都是承诺式「松手才执行」：拖动全程零写窗，物理按键监视器确认松手后再做几何事务。贴边态的书签头是独立恒尺寸窗口（窗口交接），可见时刻不改透明窗尺寸。几何阈值与矩形计算在 `shared/note-dock.ts`，持久化只写 `NoteRecord.dock?: { side, y, x? }`；交互、交接时序与验收边界以 [`docs/design/edge-dock.md`](design/edge-dock.md) 为真源。
