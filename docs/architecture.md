...
收起横条还有第三态「贴边」：窗口状态机为 `expanded → collapsed（40px 横条）→ docked（左右工作区边缘的 96×32 横条彩色书签头，有名字横排露出一小段）`，由同一个收起控制器编排，复用「renderer 演视觉、主进程只在过渡两端各做一次非动画 `setBounds`」的约束；横条→贴边先演收缩视觉再缩窗口，贴边→展开先放大窗口再演生长视觉，且同样禁止 `setBounds(..., true)` 与 `.note-shell` 基础态尺寸过渡。贴边只允许从横条进入，展开只允许从贴边进入，不支持展开直贴或贴边回横条。展开态横条的窗口拖动仍走 `-webkit-app-region: drag`；收起横条与贴边书签头不走 app-region，拖动是事件驱动：renderer 指针越过 3px 阈值后发一次 `start-note-window-drag`（带按下点相对窗口的抓取偏移），拖动中每个 `pointermove` 发一次 `move-note-window-drag`（带光标屏幕坐标），主进程逐事件 `setPosition`——与原生拖窗同源，比固定间隔轮询 `getCursorScreenPoint` 跟手（不错相、无起步空窗）；松手（`pointerup` / `pointercancel`）发一次 `finish-note-window-drag`（事件自带屏幕坐标），主进程补最后一段位移后才判定。三条通道全部 fire-and-forget（`ipcRenderer.send` / `ipcMain.on`），同一管道保序。移窗只 `setPosition` 不 `setBounds`（逐帧 `setBounds` 带尺寸、每帧触发布局，是更早一版卡顿的根因），也不存在「拖动中的停顿被当成松手」：macOS 的 `moved` 是 `move` 的别名（拖动中连续触发），平台没有任何「拖动结束」窗口事件，基于 `moved` 静默期的近似不允许回来。「松手是否贴边（左右缘 24px 阈值，顶底不吸）」与「书签头拖出是否展开（水平 48px 阈值，不足则弹回原位）」因此只在 `finish-note-window-drag` IPC 上由主进程判定一次（每窗一份 `main/note-window-drag.ts` 会话）。判定通过后经 `dock-offer` / `undock-offer` 通知 renderer 播 240ms 过渡，renderer 再带 epoch `accept-dock` / `accept-undock` 确认：会话只认最后一次 offer，重新拖动即刻作废旧 offer，accept-dock 时主进程复核横条仍在该侧边缘，展开矩形永远由主进程在松手时算好（renderer 传值不参与），主进程此时才落原生矩形并持久化。几何阈值与矩形计算全部在 `shared/note-dock.ts`，主进程与测试共用。持久化只写 `NoteRecord.dock?: { side, y }`；`bounds` 始终表示最后一次展开态的位置与尺寸，进入贴边、拖动缝、弹回贴边都不改写 `bounds`，从贴边展开后用松手位置夹取过的展开矩形写回并删除 `dock`。启动时合法 `dock` 直接按 96×32 建窗（`minWidth/minHeight` 用 96/32 而非 `NOTE_MIN_WIDTH`），书签头夹取后不落在任何可见工作区则丢弃 `dock` 按 `bounds` 展开；建贴边窗时主进程把 side 写进页面 URL query，preload 同步暴露，renderer 首个 state 初始值就是贴边态——首帧 DOM 就是书签头，不会先挂完整便签再切换。多张同侧书签头即将重叠时沿边向下错开 40px（签高 32 + 8 间隙），不够再向上，仍无空位保证至少 24px 可抓高度。贴边不做截图排除 API、悬停探出、快捷键或吸顶/底边；设计真源是 [`docs/design/edge-dock.md`](design/edge-dock.md)。
