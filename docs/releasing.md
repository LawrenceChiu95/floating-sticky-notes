...
10. 立即用上一正式版本验证线上更新。Windows 使用足够慢的测试下载或限速代理，完成以下检查：
    ...
    - 为便签命名后，确认名称持久化并出现在任务栏和 Alt-Tab；空名称回退为“悬浮便签”。检查顶部中央的命名相关 UI 只在命名热区悬停、编辑或保存后约 1.5 秒确认期间出现，普通双击进入编辑且不影响抓手和其余顶栏拖动。
    - 选择“重启并安装”后完成退出和安装，并确认本地便签与图片保留。
11. Mac 应完成检查、下载、SHA-512 校验、打开 DMG 和退出，并由用户拖入 Applications 完成替换，同时确认本地便签保留；还要验证顶部中央普通单指双击进入命名而不触发系统标题栏缩放，名称保存、取消、悬停显隐和窗口拖动符合预期。
